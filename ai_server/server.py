...
# Define the media directory path for image files
MEDIA_DIRECTORY = Path(__file__).resolve().parent / "media"
...
def process_image():
    """
    Endpoint to handle image processing based on image file name.
    Expects a JSON payload with an 'image_name' field.
    Should try to refactor these together at some point
    """
    ...
    if not os.path.isfile(image_path):
        return jsonify({"error": f"Image '{image_name}' not found in media directory"}), 404
    ...
    try:
        ...
        # Create the prompt for the AI
        prompt = f"""
        You are a terrain generation AI for a game. Based on the visual content of the uploaded image, 
        return a JSON object with parameters required for generating terrain. 
        Make sure that the values for each parameter fall within reasonable ranges to avoid any out-of-bounds issues. 
        If the description for the terrain does not need tree or grass like dessert(height 162, octaves 8, ) or snow unles the user specicly said, do not add( make the values zero's).
        Make sure if the user say dont add <object> make sure to set the value to zero, for exampl if the user say dont add tree or grass or water(etc) make it zero.
        ...
        Use the following image to generate appropriate values:
        "{image_name}"
        """

        # Generate content based on the uploaded image reference
        response = model.generate_content([uploaded_file, prompt])
        
        # Parse and return the response
        result = response.text.strip().strip('```json').strip('```')
